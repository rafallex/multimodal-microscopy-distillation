...
\begin{enumerate}
  ...
  \item \textbf{An empirical characterization of iterative noisy student~\cite{xie2020noisystudent} in this regime.} Round 1 (hard pseudo $\rightarrow$ soft pseudo / all 59k cells) returned $+0.039$ LB; round 2 (soft $\rightarrow$ soft with a stronger teacher) returned $+0.003$ LB on the ensemble but $+0.091$ LB on the best single seed --- the round-2 mean lift compressed $\sim$14$\times$ while the seed dispersion expanded $\sim$3$\times$.
...
Fig.~\ref{fig:lb} plots the chronological public-LB sequence with the most consequential inflections annotated. The arc is: v19 baseline at LB $0.7455$ (EfficientNet-B0 dual-branch + MIL + AdaBN + test stain normalization); v41 at $0.7563$ ($+0.011$, four-component L4-regularizer stack); v44 at $0.7812$ ($+0.025$ over v41, hard pseudo-labels added); v46 at $0.8236$ ($+0.039$ over its lucky-seed teacher, soft pseudo-labels replacing hard); v47 at $0.8264$ for the ensemble and $0.8355$ for the best single seed ($+0.091$ over v46, iterative noisy student round 2). The cumulative lift from baseline to best single-seed v47 is $+0.090$ LB; the ensemble of v47 holds first place on the public leaderboard with a $+0.013$ margin over the next competitor.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.95\textwidth]{lb_progression.pdf}
  \caption{Public-LB progression across 30 logged iterations. Color encodes outcome category: green for breakthrough versions (v41, v44, v46, v47), blue for useful gains, red for regressions and negative results, gray for neutral / replication runs, orange for queued versions. The reference dotted line marks the v19 baseline at $0.7455$; the dashed line marks the prior LB leader at $0.7916$. v47 takes the top of the chart at $0.8264$ (ensemble); the per-seed dispersion of v47 (seed range $0.8126$--$0.8355$) is discussed in \S\ref{sec:negative}-G.}
  \label{fig:lb}
\end{figure*}
...
\begin{table*}[!t]
  ...
  \begin{tabular}{@{}lp{8.5cm}rr@{}}
    ...
    v45p & sigmoid-avg of v41$+$v44 & 0.7729 & $\downarrow$ (\S\ref{sec:negative}-F) \\
    ...
    v47\_s2 & v47 single seed (best) & \textbf{0.8355} & \textbf{+0.091} \\
...
v47 differs from v46 by exactly one variable: the pseudo-label CSV is v46's ensemble output (LB $0.8236$) instead of v44\_seed1's (LB $0.7844$). On the ensemble, round 2 lifts LB by $+0.003$ --- substantially compressed from round 1's $+0.039$. The per-seed picture is more interesting and is summarized in Table~\ref{tab:seedspread}: the spread between best and worst seed widens from v46's $0.0072$ to v47's $0.0229$, and the v47 ensemble underperforms its best single seed by $-0.0091$. We return to the within-recipe ensemble failure in \S\ref{sec:negative}-G.
...
\begin{table}[!t]
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    \midrule
    Seed range & --- & --- & --- \\
    v46 & \multicolumn{3}{r}{$0.0072$} \\
    v47 & \multicolumn{3}{r}{$0.0229$ \quad ($\sim$3$\times$ wider)} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{Setup.} Sigmoid-average of v19 ($0.7455$) and v21 ($0.7018$), under three averaging schemes (probability average, rank average, geometric mean).

\textbf{Result.} All three schemes underperformed v19 alone: $0.7422 / 0.7436 / 0.7328$ respectively.
...
\textbf{Result.} Per-seed public LB: seed 1 $0.8150$, seed 2 $\mathbf{0.8355}$, seed 3 $0.8126$ (range $0.0229$). The 3-seed ensemble landed at $0.8264$, \emph{below} the best single seed by $0.0091$. By contrast, v46's per-seed range was $0.0072$ and the v46 ensemble at $0.8236$ beat each individual seed.
...
\begin{table}[!t]
  ...
  \begin{tabular}{@{}lp{4.5cm}c@{}}
    ...
    v45\_p & Cross-recipe ensemble fails at $0.025$ gap & yes \\
...
\emph{The mean compresses, the per-seed dispersion expands.} The v47 per-seed LB range ($0.8126$--$0.8355$, span $0.0229$) is $3.2\times$ wider than v46's ($0.8157$--$0.8229$, span $0.0072$). This is the inverse of what an earlier draft of this paper hypothesized (``round 2 acts as a variance reducer''); the training-AUC tightened but the generalization spread widened. The empirically correct framing is that round 2 with a much-stronger teacher creates a higher-variance student distribution, with seed-2 landing on a favorable mode of that distribution.

\emph{The diminishing-returns regime starts after one round.} Round 1 unlocks the calibration channel that hard pseudo discards; subsequent rounds re-iterate the same channel with a marginally better-calibrated teacher. To extract a further mean lift one would need to change the mechanism, not the iteration count --- for instance, by softening the teacher distribution via Hinton's temperature parameter (v48, queued at $T=2$) or by introducing student-side input/dropout noise more aggressively than v47 does.
...
\textbf{Single public-LB number per submission.} Each LB delta in this paper is a single noisy estimate from one public-LB split. The v47 per-seed spread of $0.023$ provides a direct measurement of the noise floor: any reported delta below $\sim$$0.01$ should be treated as not reliably distinguishable from zero. The $+0.011$ v19$\rightarrow$v41 gain is just above this floor; the $+0.039$ v44$\rightarrow$v46 gain is well above it.
...
Across 30 logged iterations on a small-sample patient-grouped multimodal-microscopy benchmark, we layered EfficientNet-B0 dual-branch architecture, MIL auxiliary loss, batch-normalization adaptation, multiscale TTA, label-smoothing and dropout regularization, hard pseudo-labels, soft-target distillation, and iterative noisy student training. The public-LB lift over a strong supervised baseline (v19 $0.7455$ $\rightarrow$ v47 ensemble $0.8264$ / best seed $0.8355$) is $+0.081$--$+0.090$. The single largest gain ($+0.039$ LB) is from soft pseudo-labels replacing hard, and is one order of magnitude larger than the cumulative regularizer-stack gain. Seven failure modes are diagnosed, including a within-recipe ensemble net-negative under an outlier seed that informs the final-pick decision.
