...
\textbf{Beyond the CNN family: an orthogonal feature model.} Diversity \emph{within} the CNN family is bounded --- an early-fusion or ResNet-50 CNN still learns convolutional features and stays correlated ($0.72$--$0.80$) with the EfficientNet base. A genuinely orthogonal member comes from a different \emph{representation}: we extract 87 hand-crafted per-cell descriptors (intensity statistics, Otsu-region morphology, GLCM and LBP texture, and a cross-modal BF--FL correlation feature) and fit a gradient-boosted tree (LightGBM). Under leave-(one-positive, one-negative)-patient-out cross-validation it reaches $0.82 \pm 0.10$ AUC --- on par with the CNN --- while its test predictions correlate only $\rho = 0.41$ (Spearman) with the best CNN seed, less than half the correlation of any CNN variant. A member that is comparably strong yet this decorrelated is, by the analysis above, the most valuable ensembling ingredient available on this benchmark. An FL-only feature model ($0.76$ AUC) also beats a BF-only one ($0.72$), confirming that fluorescence carries the dominant single-modality signal while the two modalities remain complementary.
