...
\paragraph{Patient-grouped structure and effective sample size.} Each patient is a bag of $\sim$10{,}000 cells, and \emph{every bag is single-class}: all 12 training patients are purely positive (5 patients) or purely negative (7 patients), so each cell simply inherits its patient's diagnosis. The 114{,}302 cell-level labels therefore encode only 12 independent bits of supervision --- the \emph{effective} sample size is 12 patients, not 114k cells. We argue this is the root cause of both the large per-seed LB variance and the weak CV--LB correlation documented throughout this paper, and it is why methods that enlarge the \emph{effective} labeled set (\S\ref{sec:methods}-G) dominate methods that polish the supervised loss. Cell-level BCE alone overfits the within-patient correlation, so we add a per-patient mean-logit BCE auxiliary loss (weight 0.5) inspired by attention-based MIL~\cite{ilse2018attentionmil}. For pseudo cells the patient identity is undefined; we set \texttt{patient\_id = -1} and skip the MIL loss for those rows.
...
\subsection{Test-time patient aggregation --- patient identity is not recoverable}

The single-class bag structure (\S\ref{sec:data}) suggests a powerful lever: if test cells could be regrouped into their source patients, assigning each cell its patient's \emph{consensus} prediction would average out per-cell noise and push per-cell AUC toward the (much higher) patient-separation ceiling. We tested whether patients are recoverable \emph{without} labels by clustering cells on malignancy-agnostic acquisition signatures --- per-channel ($R,G,B$) means and standard deviations, plus grayscale intensity percentiles, of the paired BF and FL crops.

\textbf{Result.} On the 12 training patients, where the ground-truth grouping is known, $K$-means and agglomerative clustering at $K \in \{12, 14, 20\}$ reach an adjusted Rand index of only $\approx 0.06$ and a mean cluster purity of $\approx 0.30$; \emph{every} recovered cluster mixes positive and negative patients.

\textbf{Diagnosis.} Patient identity is not encoded in global image statistics: the acquisition is well stain- and illumination-normalized across patients, leaving no batch signature to cluster on. Test-time patient aggregation is therefore infeasible on this benchmark, and na\"ive aggregation over mis-recovered clusters would pool cells of opposite classes and \emph{degrade} the score. This is a useful boundary result --- it explains why no submission approaches the patient-separation ceiling, and confirms that gains must come from better per-cell discrimination or decorrelated ensembling (\S\ref{sec:discussion}-D), not from recovering the latent bag structure at test time.
...
\textbf{Implication for future work: diversify the backbone, not the seed.} Every strong model in this study shares the EfficientNet-B0 dual-branch backbone, so their predictions are highly correlated (rank-correlation $>0.94$ among v44/v46/v47) --- which is precisely why symmetric averaging never beats the best single seed here. The route to a net-positive ensemble is therefore \emph{architectural} diversity: members that are individually strong but decorrelated. A rank-correlation analysis over our archived submissions flagged two candidate decorrelated bases --- an \emph{early-fusion} single backbone on the stacked 2-channel input (rank-corr $0.72$ with our best model) and a \emph{ResNet-50} backbone ($0.80$ when supervised-only). Re-training the ResNet-50 under the full soft-pseudo recipe, however, \emph{underdelivered}: it reached only LB $0.79$ and its rank-correlation with the best EfficientNet seed \emph{rose} to $0.90$ --- distilling both backbones from the same teacher made them \emph{more} alike, not less. The lesson is that diversity \emph{within} the CNN family is bounded; the genuinely decorrelated member comes from outside it (next subsection).

\textbf{Beyond the CNN family: an orthogonal feature model.} Diversity \emph{within} the CNN family is bounded --- an early-fusion or ResNet-50 CNN still learns convolutional features and stays correlated ($0.72$--$0.80$) with the EfficientNet base. A genuinely orthogonal member comes from a different \emph{representation}: we extract 87 hand-crafted per-cell descriptors (intensity statistics, Otsu-region morphology, GLCM and LBP texture, and a cross-modal BF--FL correlation feature) and fit a gradient-boosted tree (LightGBM). Under leave-(one-positive, one-negative)-patient-out cross-validation it reaches $0.82 \pm 0.10$ AUC --- on par with the CNN --- while its test predictions correlate only $\rho = 0.41$ (Spearman) with the best CNN seed, less than half the correlation of any CNN variant. On paper this is the ideal ensembling ingredient --- comparably strong yet maximally decorrelated. \emph{It was not.} The GBM's cross-validation did not transfer to the held-out test patients: a 50/50 rank-average of the GBM with our best model scored only LB $0.7074$, implying the GBM is $\approx$$0.60$ on the true test despite its $0.82$ CV. This is the same CV-versus-LB overfit that sank the contrastive SSL pretraining (\S\ref{sec:negative}-D), now sprung on the ensemble itself, and it is decisive: \emph{even a genuinely decorrelated member --- the textbook-ideal ensembling case --- regressed below the single best model.} Taken together with the within-recipe seed-averaging regression (v47 3-seed mean $0.8264 <$ best seed $0.8355$) and the cross-recipe regressions (v22, v45\_probe), the verdict on this benchmark is unambiguous: \textbf{no ensemble of any kind beat the best single seed.} The high per-patient variance on $N=12$ means averaging regresses toward a worse mean rather than cancelling noise, so the final submission is a single well-chosen model, never a blend. The one surviving single-modality observation is that an FL-only feature model ($0.76$ CV) beats a BF-only one ($0.72$), consistent with fluorescence carrying the dominant signal.

\textbf{The architecture this culminates in.} The progression early $\rightarrow$ late $\rightarrow$ orthogonal fusion points toward the design the source-dataset authors report as state of the art: \emph{intermediate} co-attention fusion (CAFNet~\cite{lian2024cafnet}), in which each modality's feature map cross-attends to the other before pooling. We implement this with a cross-attention block replacing the late-concat head; informed by the ResNet-50 result above, we apply it to the \emph{proven} EfficientNet-B0 backbone rather than the paper's ResNet-50, and harden the attention for mixed precision (fp32 attention, pre-norm, and a zero-initialized ReZero gate so the block begins as a no-op and the network starts as stable as the bare pretrained backbone). \textbf{The result closes the argument:} on the leaderboard the co-attention model scored $0.8314$ (3-seed mean) with rank-correlation $0.97$ to the late-fusion model --- the SOTA fusion produced essentially the \emph{same} predictor as a plain concatenation, neither helping nor hurting. Taken with the measured late-fusion result ($0.8264$--$0.8355$) and the ResNet-50 result ($0.79$, rank-corr $0.90$), the pattern is clear: late and intermediate co-attention fusion on the EfficientNet-B0 backbone land within noise of $0.83$ and correlate above $0.96$, while a different backbone is weaker but no more decorrelated. With twelve effective patient-labels the task is data-bound: on this benchmark \emph{architecture is not the lever} --- only the test-set distillation moved it.
