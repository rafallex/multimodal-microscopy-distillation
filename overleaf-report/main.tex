...
\textbf{The architecture this culminates in.} The progression early $\rightarrow$ late $\rightarrow$ orthogonal fusion points toward the design the source-dataset authors report as state of the art: \emph{intermediate} co-attention fusion (CAFNet~\cite{lian2024cafnet}), in which each modality's feature map cross-attends to the other before pooling. We implement this with a cross-attention block replacing the late-concat head; informed by the ResNet-50 result above, we apply it to the \emph{proven} EfficientNet-B0 backbone rather than the paper's ResNet-50, and harden the attention for mixed precision (fp32 attention, pre-norm, and a zero-initialized ReZero gate so the block begins as a no-op and the network starts as stable as the bare pretrained backbone). A full leaderboard evaluation is left to future work.
